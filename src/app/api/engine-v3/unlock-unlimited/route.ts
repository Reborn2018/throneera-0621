...
const unlockUnlimitedSchema = z.object({
...
export async function POST(request: Request) {
  const parsed = unlockUnlimitedSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid engine-v3 unlimited unlock payload" }, { status: 400 });
  }
  ...
  if (terminalState.phase !== "terminal") {
    return NextResponse.json({ error: "Unlimited unlock requires a terminal state" }, { status: 400 });
  }

  const unlimitedTerminalState: GameState = {
  ...
  const gameState = createNextGenerationState(unlimitedTerminalState, dynastyRecord);
